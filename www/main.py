...
async def add_notification(user_id, noti_type, message):
    ...
    # 💡 DB에 notification 배열이 깨져있어도 강제로 복구 후 푸시
    try:
...
# 🔥 백엔드 500 에러를 원천 차단하는 가장 강력한 형태의 팔로우 로직
...
async def toggle_follow(target_user: str, request: Request):
    ...
    # 2. 파이썬 단에서 완벽하게 리스트 편집 ($pull, $addToSet 미사용 -> DB 충돌 제로)
    if is_following:
    ...
    # 4. 상대방의 팔로워 숫자 안전하게 계산 후 덮어쓰기 ($inc 미사용)
    target = await users_col.find_one({"user_id": target_user})
    if target:
        current_followers = target.get("followers", 0)
        if type(current_followers) is not int: current_followers = 0
        
        if is_following:
            new_followers = max(0, current_followers - 1) # 언팔로우 (음수 방지)
        else:
            new_followers = current_followers + 1 # 팔로우
            
        await users_col.update_one({"user_id": target_user}, {"$set": {"followers": new_followers}})
        
        # 팔로우 알림 (에러나도 진행되도록 설계)
        if not is_following:
            await add_notification(target_user, "follow", f"🤝 {user_id}님이 회원님을 팔로우합니다.")

    return {"message": "success", "following": following_list}
